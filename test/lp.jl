...
@testset "LocalProjectionResult" begin
    ...
    r = LocalProjectionResult(B, V, [1], nothing, HRVCE(), VarName[:y1, :y2], VarName[:x], VarName[:w1, :w2], Dict{VarName,Int}(:y1=>1, :y2=>2), Dict{VarName,Int}(:x=>1), Dict{VarName,Int}(:w1=>1,:w2=>2,:y1=>3,:y2=>4), 2, 0, nothing, nothing, nothing, nothing, nothing, false, true)
    @test coef(r, 1, :x) == B[1][1,1]
    @test coef(r, 2, :w1, yname=:y2, lag=1) == B[2][2,2]
    @test coef(r, 2, :w1, lag=2) == B[2][4,1]
    @test vcov(r, 1, :x) == V[1][1,1]
    @test vcov(r, 2, :x, yname1=:y2, xwname2=:w1, lag2=1) == V[2][2,4]
    @test vcov(r, 2, :x, yname1=:y2, xwname2=:w1, yname2=:y1, lag2=1) == V[2][2,3]
    @test vcov(r, 2, :w1, yname1=:y2, lag1=2, xwname2=:w1, yname2=:y1, lag2=1) == V[2][8,3]
    @test vcov(r, 2, :w2, yname1=:y2, lag1=2, xwname2=:w1, yname2=:y1) == V[2][10,7]
    ...
    @test sprint(show, MIME("text/plain"), r) == """
        LocalProjectionResult with 2 lags over 2 horizons:
          outcomes: y1 y2
          regressor: x
          lagged controls: w1 w2"""
    r = LocalProjectionResult([ones(1,1)], [ones(1,1)], [1], nothing, HRVCE(), VarName[:y], VarName[], VarName[:w], Dict{VarName,Int}(:y=>1), Dict{VarName,Int}(), Dict{VarName,Int}(:w=>1), 1, 0, nothing, nothing, nothing, nothing, nothing, false, true)
    @test sprint(show, MIME("text/plain"), r) == """
        LocalProjectionResult with 1 lag over 1 horizon:
          outcome: y
          lagged control: w"""
    r = LocalProjectionResult([ones(1,1)], [ones(1,1)], [1], nothing, HRVCE(), VarName[Cum(:y)], VarName[Cum(:x)], VarName[:w], Dict{VarName,Int}(Cum(:y)=>1), Dict{VarName,Int}(Cum(:x)=>1), Dict{VarName,Int}(:w=>1), 1, 0, nothing, nothing, nothing, VarName[Cum(:x)], VarName[:z1,:z2], false, true)
    @test sprint(show, MIME("text/plain"), r) == """
        LocalProjectionResult with 1 lag over 1 horizon:
          outcome: Cum(y)
          regressor: Cum(x)
          lagged control: w
          endogenous variable: Cum(x)
          instruments: z1 z2"""
end
